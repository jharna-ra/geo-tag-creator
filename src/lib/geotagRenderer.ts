...
export function formatDateLine(
  ...
  const ddmmyyyy = `${String(d).padStart(2, "0")}/${String(m).padStart(
    2,
    "0"
  )}/${y}`;
 ...
 * └──────────────────────────────────────────┘
 *
 * The banner has SQUARE corners.
 *
 * It works whether or not geocoding successfully
 * returned city/state/country.
 */
  ...
  if (
    ...
    try {
      const map = await renderMapCanvas(
        lat,
        lon,
        Math.max(mapW, height),
        opts.mapType,
        15
      );

      /*
       * Draw map on the left side.
       */
      ctx.drawImage(
        map,
        0,
...
/**
 * Converts the generated geotag canvas
 * into a PNG Blob.
 */
